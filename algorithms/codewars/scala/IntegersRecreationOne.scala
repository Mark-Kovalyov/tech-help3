/*
 ...
 these divisors we get: 1, 60516, 4, 15129, 9, 6724, 36, 1681.
...
Task
 ...
 first the number the squared divisors of which is a square and then the
...
object ROne{
  ...
  def is_prime(x : Int) : Boolean = !(2 to 1 + Math.sqrt(x).toInt).map(i => x % i).exists(_ == 0)

	def list_squared(m: Int, n: Int) : List[(Int,Int)] = {
	  require(1 <= m)
		require(m <= n)
		for(i <- m to n) {
